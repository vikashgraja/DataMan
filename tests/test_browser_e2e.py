...
async def _run_playwright_e2e(tmp_path):
    ...
    if env_file.exists():
        env_file.write_text(
            env_file.read_text().replace("ALLOWED_HOSTS=\n", "ALLOWED_HOSTS=*\n")
        )
...
"""
    ...
    with open(server_log_file, "w", encoding="utf-8") as server_log:
        ...
        try:
            assert wait_for_server(port, timeout=15), (
                f"DataMan server failed to start on port {port}"
            )
            ...
            async with async_playwright() as p:
                ...
                await page.goto(
                    f"{base_url}/admin/login/", wait_until="domcontentloaded"
                )
                ...
                await page.wait_for_url(
                    "**/admin/", wait_until="domcontentloaded", timeout=10000
                )
                ...
                assert len(revealed_key) > 10, (
                    f"Expected valid token string, got '{revealed_key}'"
                )
